...
Purpose: evaluate how different machine learning models perform on the training dataset without outliers (classify bankrupt companies)
"""
...
X_train = pd.read_csv(r"inliers\X_all_train_wo_os.csv", low_memory=False, index_col = 0).iloc[:, 2:]
y_train = pd.read_csv(r"inliers\y_train_wo_os.csv", low_memory=False, index_col = 0)["bnkrpt"].squeeze()
X_test = pd.read_csv(r"inliers\X_all_test.csv", low_memory=False, index_col = 0).iloc[:, 2:]
y_test = pd.read_csv(r"inliers\y_test.csv", low_memory=False, index_col = 0)["bnkrpt"].squeeze()
dataname = "inliers"
